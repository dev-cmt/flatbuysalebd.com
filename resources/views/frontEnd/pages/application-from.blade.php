...
@section('content')
                                ...
                                <div class="col-lg-8 mx-auto">
                                    <div class="card border-0 shadow-sm">
                                        <div class="card-body text-center p-4">
                                            ...
                                            <div class="mb-4">
                                                <!-- Logos in a flex row -->
                                                <div class="d-flex flex-wrap gap-3 justify-content-center align-items-center" id="payment-logos">
                                                    @foreach ($paymentMethods as $m)
                                                        <img src="{{ asset($m->logo) }}" alt="{{ $m->name }}"
                                                            class="img-fluid rounded shadow-sm toggle-qr p-2"
                                                            style="max-width:120px;cursor:pointer"
                                                            data-id="{{ $m->id }}"
                                                            data-image="{{ $m->qr_code ? asset($m->qr_code) : '' }}"
                                                            data-instructions="{{ $m->instructions ?? '' }}">
                                                    @endforeach
                                                </div>

                                                <!-- hidden field for selected payment id -->
                                                <input type="hidden" name="payment_id" id="payment_id">

                                                <!-- QR box – always visible -->
                                                <div id="qr-area" class="text-center mt-4 border rounded p-3" style="min-height:200px;">
                                                    <img id="qr-img" class="img-fluid mb-2 d-none" style="max-width:150px;">
                                                    <p id="qr-instructions" class="small text-muted d-none"></p>
                                                </div>
                                            </div>
                                            @else
                                            <div class="mb-4">
    ...
    <script>
        $(document).on('click', '.toggle-qr', function () {
            $('.toggle-qr').removeClass('border border-primary');
            $(this).addClass('border border-primary');
            $('#payment_id').val($(this).data('id'));

            const url = $(this).data('image'),
                inst = $(this).data('instructions');

            $('#qr-img').toggleClass('d-none', !url).attr('src', url || '');
            $('#qr-instructions')
                .text(url ? (inst || '') : 'No QR code available')
                .toggleClass('d-none', url && !inst);
        });
    </script>
